...
local esp = {
    ...
    --\ @objects
    object_names = { false, Color3.new()},
    object_distance = false,
...
function esp:update()
    for plr, drawing in next, esp.players do
        ...
        if esp.enabled and esp.checkalive(player) then
            ...
            local kevlar = 0
            if character:FindFirstChild('BodyEffects') and character:FindFirstChild('BodyEffects'):FindFirstChild('Armor') then
                kevlar = character:FindFirstChild('BodyEffects'):FindFirstChild('Armor').Value
            end
            local health = FLOOR(character.Humanoid.Health)
            
            local weapon
            if character:FindFirstChildOfClass('Tool') then
            	weapon = character:FindFirstChildOfClass('Tool').Name
            end
            ...
            if drawing.arrow.Visible then
                ...
                if esp.arrowinfo then
                    ...
                    if drawing.arrow_kevlarbar.Visible then
                        drawing.arrow_kevlarbar.Color = esp[ flag .. 'kevlarbar'][3]:Lerp(esp[ flag .. 'kevlarbar'][2], kevlar / 200)
                        drawing.arrow_kevlarbar.Size = esp:floorvector(NEWVEC2(( kevlar / 200 * ( biggestX - smallestX)), 1))
            ...
            if drawing.kevlarbar.Visible then
                drawing.kevlarbar.Color = esp[ flag .. 'kevlarbar'][3]:Lerp(esp[ flag .. 'kevlarbar'][2], kevlar / 200)
                drawing.kevlarbar.Size = esp:floorvector(NEWVEC2(( kevlar / 200 * ( biggestX - smallestX)), 1))
            ...
            if drawing.weapon.Visible then
                drawing.weapon.Text = weapon or 'none'
    ...
    for obj, drawing in next, esp.objects do
        ...
        drawing.distance.Visible = not esp['object_names'][1] and esp['object_distance']
        drawing.distance_outline.Visible = esp.outlines and drawing.distance.Visible
        if drawing.distance.Visible then
            drawing.distance.Text = '['..distance..']'
            drawing.distance.Font = Drawing.Fonts[esp.font]
